...
const ChatSidebar: React.FC<ChatSidebarProps> = memo(({ isOpen, onClose, isDesktop = false }) => {
  ...
  // (polling, post-delete, post-send) so the list doesn't flicker.
  ...
  useEffect(() => {
    ...
    // Titles are generated server-side a moment after the first exchange
    // in a conversation, and a brand-new conversation won't exist in this
    // list at all until it's saved. Refetch quietly so the sidebar catches
    // up without the user having to reload the page.
    const handleExternalRefresh = () => fetchConversations(true);
    window.addEventListener('conversationsUpdated', handleExternalRefresh);

    const pollId = window.setInterval(() => fetchConversations(true), 20000);

    return () => {
      window.removeEventListener('conversationsUpdated', handleExternalRefresh);
  ...
  const handleDeleteConversation = async (
    ...
    if (confirmingId !== conversationId) {
      // First click just arms the confirm state; second click (or the
      // check button) actually deletes. Avoids a native confirm() popup.
    ...
    try {
      ...
      // If the conversation being deleted is the one currently open, back
      // out to a fresh chat so the user isn't left looking at a dead thread.
              ...
              {loading
                ...
                : filteredConversations.length > 0
                  ? filteredConversations.map((conv) => (
                      <div
                        key={conv.id}
                        className={`relative flex items-center rounded-xl group ${isOpen ? 'w-full' : 'w-10 justify-center'} ${theme === 'dark' ? 'hover:bg-[#1e2329]' : 'hover:bg-gray-200'}`}
                        onMouseLeave={() =>
                          setConfirmingId((id) => (id === conv.id ? null : id))
                        }
                      >
                        <button
                          ...
                          className={`${isOpen ? 'flex-1 min-w-0 pl-2.5 pr-1' : 'w-10 justify-center'} flex items-center space-x-3 py-2.5 transition-all duration-200 disabled:opacity-40 ${theme === 'dark' ? 'text-gray-400 group-hover:text-gray-200' : 'text-gray-500 group-hover:text-gray-800'}`}
                        >
                        ...
                        {isOpen && token && (
                          <>
                            {confirmingId === conv.id ? (
                              <div className="flex items-center shrink-0 pr-2 space-x-1">
                                <button
                                ...
                                <button
                                  ...
                                  title="Cancel"
                                  className={`p-1.5 rounded-lg ${theme === 'dark' ? 'text-gray-400 hover:bg-gray-700' : 'text-gray-500 hover:bg-gray-300'}`}
                                >
                              ...
                              <button
                                ...
                                disabled={deletingId === conv.id}
                                title="Delete chat"
                                className={`shrink-0 mr-2 p-1.5 rounded-lg opacity-0 group-hover:opacity-100 transition-opacity disabled:opacity-40 ${theme === 'dark' ? 'text-gray-500 hover:text-red-400 hover:bg-red-500/10' : 'text-gray-400 hover:text-red-500 hover:bg-red-50'}`}
                              >
                                <FiTrash2 className="w-4 h-4" />
                              </button>
                            )}
