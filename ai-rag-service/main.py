...
class HTMLToMarkdownConverter(HTMLParser):
    ...
    def handle_starttag(self, tag, attrs):
        ...
        # Strikethrough
        elif tag == 'strike' or tag == 'del' or tag == 's':
        ...
        # Lists
            ...
            self.list_stack.append('ul' if tag == 'ul' else 'ol')
            self.result.append('\n')
            ...
            if self.list_stack and self.list_stack[-1] == 'ol':
                # Numbered list - we'll handle numbering during close
    ...
    def handle_data(self, data):
        if not data.strip() and not self.in_table_cell:
            return
        ...
        # If we're in a link, format properly
        if self.in_link and self.link_href:
            # Clean the URL
            url = self.link_href.strip()
    ...
    def _render_table(self):
        """Render a Markdown table from collected data."""
...
def convert_html_to_markdown(html_text: str) -> str:
    ...
    # First, handle common patterns that break the parser
    
    # Replace <br> with newline
    html_text = re.sub(r'<br\s*/?>', '\n', html_text, flags=re.IGNORECASE)
    
    # Replace <li> with list markers
    html_text = re.sub(r'<li\s*/?>', '- ', html_text, flags=re.IGNORECASE)
    
    # Replace <ul> and <ol> with newlines
    html_text = re.sub(r'<[/]?(ul|ol)\s*/?>', '\n', html_text, flags=re.IGNORECASE)
    
    # Remove empty tags
    html_text = re.sub(r'<[^>]+>\s*</[^>]+>', '', html_text, flags=re.IGNORECASE)
    ...
    # Handle italic
    html_text = re.sub(r'<i>', '*', html_text, flags=re.IGNORECASE)
    html_text = re.sub(r'</i>', '*', html_text, flags=re.IGNORECASE)
    html_text = re.sub(r'<em>', '*', html_text, flags=re.IGNORECASE)
    html_text = re.sub(r'</em>', '*', html_text, flags=re.IGNORECASE)
    
    # Handle underline
    html_text = re.sub(r'<u>', '_', html_text, flags=re.IGNORECASE)
    html_text = re.sub(r'</u>', '_', html_text, flags=re.IGNORECASE)
    
    # Handle strikethrough
    html_text = re.sub(r'<strike>', '~~', html_text, flags=re.IGNORECASE)
    html_text = re.sub(r'</strike>', '~~', html_text, flags=re.IGNORECASE)
    html_text = re.sub(r'<del>', '~~', html_text, flags=re.IGNORECASE)
    html_text = re.sub(r'</del>', '~~', html_text, flags=re.IGNORECASE)
    html_text = re.sub(r'<s>', '~~', html_text, flags=re.IGNORECASE)
    html_text = re.sub(r'</s>', '~~', html_text, flags=re.IGNORECASE)
    
    # Handle headings
    html_text = re.sub(r'<h1>', '\n# ', html_text, flags=re.IGNORECASE)
    html_text = re.sub(r'</h1>', '\n', html_text, flags=re.IGNORECASE)
    html_text = re.sub(r'<h2>', '\n## ', html_text, flags=re.IGNORECASE)
    html_text = re.sub(r'</h2>', '\n', html_text, flags=re.IGNORECASE)
    html_text = re.sub(r'<h3>', '\n### ', html_text, flags=re.IGNORECASE)
    html_text = re.sub(r'</h3>', '\n', html_text, flags=re.IGNORECASE)
    html_text = re.sub(r'<h4>', '\n#### ', html_text, flags=re.IGNORECASE)
    html_text = re.sub(r'</h4>', '\n', html_text, flags=re.IGNORECASE)
    html_text = re.sub(r'<h5>', '\n##### ', html_text, flags=re.IGNORECASE)
    html_text = re.sub(r'</h5>', '\n', html_text, flags=re.IGNORECASE)
    html_text = re.sub(r'<h6>', '\n###### ', html_text, flags=re.IGNORECASE)
    html_text = re.sub(r'</h6>', '\n', html_text, flags=re.IGNORECASE)
    ...
    # Handle blockquote
    html_text = re.sub(r'<blockquote>', '\n> ', html_text, flags=re.IGNORECASE)
    html_text = re.sub(r'</blockquote>', '\n', html_text, flags=re.IGNORECASE)
    
    # Handle code
    html_text = re.sub(r'<code>', '`', html_text, flags=re.IGNORECASE)
    html_text = re.sub(r'</code>', '`', html_text, flags=re.IGNORECASE)
    html_text = re.sub(r'<pre>', '\n```\n', html_text, flags=re.IGNORECASE)
    html_text = re.sub(r'</pre>', '\n```\n', html_text, flags=re.IGNORECASE)
    ...
    # Handle line breaks
    html_text = re.sub(r'\n\s*\n\s*\n', '\n\n', html_text)  # Remove excessive newlines
    
    # Remove any remaining HTML tags (with content preservation)
...
def strip_stray_html_tags(text: str) -> str:
    """
    Comprehensive HTML sanitizer that converts HTML to clean Markdown.
    Handles <br>, <li>, <b>, <i>, <table>, <tr>, <td>, <h1>-<h6>, etc.
    """
    if not text:
        return text
    
    # First, try the full HTML to Markdown converter
    try:
        converted = convert_html_to_markdown(text)
        if converted and '<' not in converted and '>' not in converted:
            return converted
    except Exception:
        pass
    
    # Fallback: strip all HTML tags but preserve structure
    ...
    # Replace table tags with plain text
    text = re.sub(r'<[/]?table[^>]*>', '\n', text, flags=re.IGNORECASE)
    text = re.sub(r'<[/]?tr[^>]*>', '\n', text, flags=re.IGNORECASE)
...
def sanitize_markdown_urls(text: str) -> str:
    """
    Sanitize markdown content - removes nested URLs, fixes malformed links,
    and strips any stray HTML tags.
    """
    if not text:
        return text

    # ========================================================================
    # STEP 1: Strip any stray HTML tags first
    # ========================================================================
    text = strip_stray_html_tags(text)

    # ========================================================================
    # STEP 2: Fix markdown links
    # ========================================================================
...
**CRITICAL FORMATTING RULE - ABSOLUTELY NON-NEGOTIABLE:**
- **NEVER use HTML tags** like <br>, <b>, <i>, <li>, <ul>, <ol>, <table>, <tr>, <td>, <h1>, <p>, etc.
- **ONLY use Markdown** for formatting: **bold**, *italic*, bullet points (`- item`), numbered lists (`1. item`), headings (`# Heading`), and links `[text](url)`
- Use **plain text with Markdown** — this is critical for the chat interface to display properly
